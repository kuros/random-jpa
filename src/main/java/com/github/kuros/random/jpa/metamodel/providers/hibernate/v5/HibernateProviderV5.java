 ...
 */
public class HibernateProviderV5 implements Provider {
    ...
    private void init() {
        final SessionFactoryImpl entityManagerFactory = (SessionFactoryImpl) entityManager.getEntityManagerFactory();

        final Map<String, EntityPersister> allClassMetadata = entityManagerFactory.getMetamodel().entityPersisters();

        final Set<EntityType<?>> entities = entityManager.getMetamodel().getEntities();
        for (EntityType<?> entity : entities) {
            final EntityPersister classMetadata = allClassMetadata.get(entity.getJavaType().getName());
